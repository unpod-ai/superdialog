...
class DialogStateMachine:
    ...
    async def _follow_router_chain(
        ...
        while not self.is_complete and hops < max_hops:
            ...
            # counter only accrues on consecutive self-loops (reset on any
            # forward transition), so legitimate multi-node chains are unaffected.
            if self.context.consecutive_self_loops >= self.context.MAX_SELF_LOOPS:
                logger.warning(
                    "[traverse] self-loop ceiling hit in chain node=%s (%d/%d) "
                    "— stopping chain",
                    node.id,
                    self.context.consecutive_self_loops,
                    self.context.MAX_SELF_LOOPS,
