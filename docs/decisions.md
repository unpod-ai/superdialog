...
| 19 | Barrier-line localization | **Authored in the playbook** (`policies.filler` / `policies.hold_line`), not host-wired. The authored fields are plain strings (`playbook/models.py::Policies`). A `PlaybookAgent(filler=, hold_line=)` argument overrides; each of those two arguments also accepts a `Callable[[ConversationState], str]` evaluated against live state, degrading to the built-in default if it raises. |
